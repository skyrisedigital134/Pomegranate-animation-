...
const Cards = () => {
  ...
  useEffect(() => {
    // ── WHY THE DELAY + REFRESH ──────────────────────────────────────────
    // The Hero pins itself for 300vh using GSAP ScrollTrigger. GSAP inserts
    // a "pin spacer" div into the DOM to account for that extra scroll height.
    // If Cards' ScrollTriggers are created BEFORE that spacer exists, they
    // calculate their trigger positions against the un-spaced document height
    // and fire immediately at page load instead of when the cards scroll into view.
    //
    // The fix: wait 500ms (enough for Hero's useEffect + pin spacer injection
    // to complete), then call ScrollTrigger.refresh() to force a full
    // recalculation of all scroll positions, then register the card triggers.
    // ─────────────────────────────────────────────────────────────────────
